...
//     })
//     .post((req, res) => {
//         req.body._id = req.params.guid || guid();
...
// Users
app.crud('recipes', '[0-9a-f]{8}\-[0-9a-f]{4}\-[0-9a-f]{4}\-[0-9a-f]{4}\-[0-9a-f]{12}');
app.crud('users', '[a-z][a-z0-9]*');

// // Users
// app.route('/api/users/:user([a-z][a-z0-9]*)')
//     .get((req, res, next) => {
...
//     })
//     .delete((req, res) => {
//         db.users.remove({_id: req.params.guid}, (err, id) => {
//             if (err)
//                 throw err;
//             if (id)
//                 res.send(id);
...
// Log in
app.get('/api/login/:user([a-z][a-z0-9]*)', (req, res) => {
    db.recipes.find({user: req.params.user}).toArray((err, docs) => {
        if (err)
            throw err;
        if (docs)
